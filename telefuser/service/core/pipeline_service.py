...
def _peak_memory_mb() -> float | None:
    try:
        ...
        return None
    peak_memory_mb = _nonnegative_finite_float(peak_bytes)
    return peak_memory_mb / _BYTES_PER_MIB if peak_memory_mb is not None else None
...
def _raw_runtime_metrics(raw: Any) -> tuple[float | None, float | None]:
    ...
    inference_time_s = _nonnegative_finite_float(raw.get("inference_time_s", metrics.get("inference_time_s")))
    peak_memory_mb = _nonnegative_finite_float(raw.get("peak_memory_mb", metrics.get("peak_memory_mb")))
    if peak_memory_mb is None:
        peak_memory_gb = _nonnegative_finite_float(raw.get("peak_memory_gb", metrics.get("peak_memory_gb")))
