"""Isolated LTX-2.5 audio VAE decoder, vocoder, and split-checkpoint loader."""
...
def _module_in_fp32(module: nn.Module, *, enabled: bool) -> Iterator[None]:
    """Temporarily materialize a module in fp32 when autocast cannot do so."""
    if not enabled:
        yield
        return
...
class VocoderWithBWE(nn.Module):
    ...
    def forward(self, mel_spec: torch.Tensor) -> torch.Tensor:
        """Decode in fp32 to match the upstream BWE accumulation contract."""
        ...
        with fp32_ctx:
            out = self.vocoder(mel_spec.float())
...
LTX25AudioVAEDecoder = AudioDecoder
LTX25AudioVocoder = Vocoder | VocoderWithBWE


def _audio_decoder_key_to_model_key(key: str) -> str | None:
...
def _vocoder_key_to_model_key(key: str) -> str | None:
    if not key.startswith("vocoder."):
        return None
    return key.removeprefix("vocoder.")


def ltx25_audio_checkpoint_key_coverage(
    checkpoint_path: str | Path,
    decoder_keys: set[str],
    vocoder_keys: set[str],
) -> tuple[set[str], set[str], set[str], set[str]]:
    """Return unexpected and missing decoder/vocoder keys without loading payloads."""
    decoder_mapped: set[str] = set()
    vocoder_mapped: set[str] = set()
    with safe_open(str(checkpoint_path), framework="pt", device="cpu") as checkpoint:
        for key in checkpoint.keys():
            if (target := _audio_decoder_key_to_model_key(key)) is not None:
                decoder_mapped.add(target)
            if (target := _vocoder_key_to_model_key(key)) is not None:
                vocoder_mapped.add(target)
    return (
        decoder_mapped - decoder_keys,
        decoder_keys - decoder_mapped,
        vocoder_mapped - vocoder_keys,
        vocoder_keys - vocoder_mapped,
    )


def load_ltx25_audio_decoder_and_vocoder(
    checkpoint_path: str | Path,
    *,
    device: torch.device | str = "cpu",
    torch_dtype: torch.dtype = torch.bfloat16,
) -> tuple[LTX25AudioVAEDecoder, LTX25AudioVocoder]:
    """Construct and strictly load the LTX-2.5 audio decoder and BWE vocoder."""
    checkpoint = inspect_checkpoint(checkpoint_path)
    with torch.device("meta"):
        decoder = AudioDecoderConfigurator.from_config(checkpoint.config)
        vocoder = VocoderConfigurator.from_config(checkpoint.config)
    decoder_unexpected, decoder_missing, vocoder_unexpected, vocoder_missing = ltx25_audio_checkpoint_key_coverage(
        checkpoint.path, set(decoder.state_dict()), set(vocoder.state_dict())
    )
    if decoder_unexpected or decoder_missing or vocoder_unexpected or vocoder_missing:
        raise ValueError(
            "LTX-2.5 audio checkpoint coverage mismatch: "
            f"decoder_unexpected={sorted(decoder_unexpected)[:5]}, "
            f"decoder_missing={sorted(decoder_missing)[:5]}, "
            f"vocoder_unexpected={sorted(vocoder_unexpected)[:5]}, "
            f"vocoder_missing={sorted(vocoder_missing)[:5]}"
        )

    decoder_state_dict: dict[str, torch.Tensor] = {}
    vocoder_state_dict: dict[str, torch.Tensor] = {}
    with safe_open(str(checkpoint.path), framework="pt", device="cpu") as source:
        for key in source.keys():
            if (target := _audio_decoder_key_to_model_key(key)) is not None:
                decoder_state_dict[target] = source.get_tensor(key)
            if (target := _vocoder_key_to_model_key(key)) is not None:
                vocoder_state_dict[target] = source.get_tensor(key)
    decoder_missing_keys, decoder_unexpected_keys = decoder.load_state_dict(
        decoder_state_dict, strict=True, assign=True
    )
    vocoder_missing_keys, vocoder_unexpected_keys = vocoder.load_state_dict(
        vocoder_state_dict, strict=True, assign=True
    )
    if decoder_missing_keys or decoder_unexpected_keys or vocoder_missing_keys or vocoder_unexpected_keys:
        raise ValueError(
            "LTX-2.5 audio weight load mismatch: "
            f"decoder_missing={decoder_missing_keys[:5]}, decoder_unexpected={decoder_unexpected_keys[:5]}, "
            f"vocoder_missing={vocoder_missing_keys[:5]}, vocoder_unexpected={vocoder_unexpected_keys[:5]}"
        )
    return decoder.to(device=device, dtype=torch_dtype).eval(), vocoder.to(device=device, dtype=torch_dtype).eval()
