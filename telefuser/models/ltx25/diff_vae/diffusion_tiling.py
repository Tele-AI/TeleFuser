...
from .types import VIDEO_SCALE_FACTORS, SpatioTemporalScaleFactors
...
# Empirical H100 guardrail for the upstream CHUNKED_COMPILE execution boundary.
# The analytical stage-5 estimate does not account for NATTEN's transient custom-op
# workspace, which can otherwise make a full-resolution tile allocate tens of GiB.
_CHUNKED_COMPILE_MAX_TILE_SIZE: tuple[int, int, int] = (80, 320, 320)
...
def _max_tile_size(mode: DiffVAEMode) -> tuple[int, int, int] | None:
    """Return the execution-mode-specific upper bound for an auto-selected tile."""
    if mode is DiffVAEMode.CHUNKED_COMPILE:
        return _CHUNKED_COMPILE_MAX_TILE_SIZE
    return None
...
def stage4_feature_bytes(
    *,
    ...
    from .types import VIDEO_SCALE_FACTORS, VideoLatentShape, VideoPixelShape  # noqa: PLC0415
...
def recommended_decode_tiling_config(  # noqa: PLR0913
    *,
    ...
    Selection (size-grid, accumulator-aware):
      ...
      3. For ``CHUNKED_COMPILE``, cap each tile to 80 frames by 320x320 pixels.
         This guards the NATTEN custom-op workspace not represented by the
         analytical estimate.
      4. Among feasible triples, pick minimal :func:`volumetric_overlap_waste`.
    Peak-bytes estimate::
    ...
    max_tile_size = _max_tile_size(mode)
    if max_tile_size is not None:
        t_cands = [candidate for candidate in t_cands if candidate[0] <= max_tile_size[0]]
        h_cands = [candidate for candidate in h_cands if candidate[0] <= max_tile_size[1]]
        w_cands = [candidate for candidate in w_cands if candidate[0] <= max_tile_size[2]]
