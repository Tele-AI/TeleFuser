...
from telefuser.models.ltx25.diff_vae.ops import PerChannelStatistics, patchify, unpatchify
from telefuser.models.ltx25.diff_vae.transformer import (
    AdaLNZero,
    ChannelLinear,
...
from telefuser.models.ltx25.diff_vae.transformer.attention import (
    NattenAttention,
    NeighborhoodAttention3D,
    configure_w_chunks,
    natten_available,
)
from telefuser.models.ltx25.diff_vae.transformer.blocks import DiffusionNABlock
from telefuser.models.ltx25.diff_vae.transformer.chunked.block import ChunkedDiffusionNABlock
from telefuser.models.ltx25.diff_vae.transformer.compiling import compile_diffusion_decoder
from telefuser.models.ltx25.diff_vae.transformer.config import DiffVAEMode
from telefuser.models.ltx25.diff_vae.transformer.fallback_na import fallback_na_attention
from telefuser.models.ltx25.diff_vae.transformer.rope_math import rope_inv_freqs
from telefuser.models.ltx25.transformer import PixArtAlphaCombinedTimestepSizeEmbeddings

from ..checkpoint import inspect_checkpoint
from . import diffusion_tiling
from .tiling import (
    Tile,
    TileSizeConfig,
    TilingConfig,
...
def cuda_activation_budget_bytes(device: torch.device) -> int:
    """Return CUDA bytes available for optional decoder tiling."""
    if device.type != "cuda":
        return 0
...
def to_velocity(sample: torch.Tensor, sigma: float | torch.Tensor, denoised_sample: torch.Tensor) -> torch.Tensor:
    """Convert an x0 prediction into the velocity parameterization."""
    sigma_value = sigma.to(torch.float32).item() if isinstance(sigma, torch.Tensor) else sigma
    if sigma_value == 0:
        raise ValueError("Sigma cannot be zero")
    return ((sample.float() - denoised_sample.float()) / sigma_value).to(sample.dtype)
...
class DiffusionVideoDecoder(nn.Module):
    ...
    def recommended_tiling_config(
        ...
        *,
        ...
        return diffusion_tiling.recommended_decode_tiling_config(
            ...
            stage5_channels=self.stage5_channels,
            stage4_channels=self.stage_channels[3],
            upsample_strides=tuple(tuple(upsample.stride) for upsample in self.upsamples),
    ...
    def from_checkpoint(
        ...
        *,
        ...
        """Construct and strictly load the standalone LTX-2.5 DiffVAE decoder."""
        checkpoint = inspect_checkpoint(checkpoint_path)
        vae_config = checkpoint.config.get("vae")
        if not isinstance(vae_config, dict) or not isinstance(vae_config.get("decoder"), dict):
            raise ValueError("LTX-2.5 DiffVAE checkpoint is missing vae.decoder config")
        ...
        kwargs = {
            ...
            "stage_kernels": tuple(tuple(value) for value in decoder_config["stage_kernels"]),
            "upsamples": tuple((tuple(value[0]), value[1]) for value in decoder_config["upsamples"]),
            "stage5_kernel": tuple(decoder_config["stage5_kernel"]),
            "default_num_inference_steps": decoder_config["default_num_inference_steps"],
            "timestep_scale_multiplier": decoder_config["timestep_scale_multiplier"],
            "model_output_type": vae_config["model_output_type"],
        }
        with torch.device("meta"):
            model = cls(**kwargs)
        unexpected, missing = ltx25_diffusion_vae_checkpoint_key_coverage(checkpoint.path, set(model.state_dict()))
        if unexpected or missing:
            raise ValueError(
                "LTX-2.5 DiffVAE checkpoint coverage mismatch: "
                f"unexpected={sorted(unexpected)[:5]}, missing={sorted(missing)[:5]}"
            )
        state_dict = _load_ltx25_diffusion_vae_state_dict(checkpoint.path)
        missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=True, assign=True)
        if missing_keys or unexpected_keys:
            raise ValueError(
                f"LTX-2.5 DiffVAE load mismatch: missing={missing_keys[:5]}, unexpected={unexpected_keys[:5]}"
            )
        model = model.to(device=device, dtype=torch_dtype)
        for module in model.modules():
            if isinstance(module, NeighborhoodAttention3D):
                # The official loader keeps these non-persistent frequency buffers
                # in FP32 even when decoder weights run in BF16.
                rope_dim_t, rope_dim_h, rope_dim_w = module.rope_dim_split
                module.rope_inv_t = rope_inv_freqs(rope_dim_t, module.rope_base).to(device=module.rope_inv_t.device)
                module.rope_inv_h = rope_inv_freqs(rope_dim_h, module.rope_base).to(device=module.rope_inv_h.device)
                module.rope_inv_w = rope_inv_freqs(rope_dim_w, module.rope_base).to(device=module.rope_inv_w.device)
        _configure_chunked_eager_mode(model)
        return model.eval()


def _configure_chunked_eager_mode(model: DiffusionVideoDecoder) -> None:
    """Apply upstream's default eager DiffVAE recipe to an isolated decoder."""
    model.deferred_stage4_upsample = True
    for block in model.diff_blocks:
        if isinstance(block, DiffusionNABlock):
            block.__class__ = ChunkedDiffusionNABlock
        block.stage4_upsample = model.upsamples[3]
        configure_w_chunks(block, w_chunks=4)

    attention = NattenAttention("cutlass-fna") if natten_available() else fallback_na_attention()
    for module in model.modules():
        if isinstance(module, NeighborhoodAttention3D):
            module.attention_function = attention
            module.natten_backend = "cutlass-fna" if natten_available() else None


def _configure_chunked_compile_mode(model: DiffusionVideoDecoder) -> None:
    """Compile chunked residuals with the conservative official cutlass-fna NATTEN backend."""
    model.deferred_stage4_upsample = True
    for block in model.diff_blocks:
        if isinstance(block, DiffusionNABlock):
            block.__class__ = ChunkedDiffusionNABlock
    ...
    for module in model.modules():
        if isinstance(module, NeighborhoodAttention3D):
            module.attention_function = NattenAttention("cutlass-fna")
            module.natten_backend = "cutlass-fna"
    compile_diffusion_decoder(model)


def _diffusion_vae_targets(key: str) -> tuple[str, ...]:
    if key.startswith("decoder."):
        target = key.removeprefix("decoder.")
        # The official checkpoint carries this vestigial tensor; NADiffusionDecoder
        # has no corresponding parameter and upstream does not consume it.
        if target == "type_emb":
            return ()
        if target.endswith(".qkv.weight"):
            prefix = target.removesuffix(".qkv.weight")
            return tuple(f"{prefix}.qkv.to_{part}.weight" for part in ("q", "k", "v"))
        if target.endswith(".qkv.bias"):
            prefix = target.removesuffix(".qkv.bias")
            return tuple(f"{prefix}.qkv.to_{part}.bias" for part in ("q", "k", "v"))
        return (
            target.replace("t_embedder.mlp.0", "t_embedder.timestep_embedder.linear_1").replace(
...
def _load_ltx25_diffusion_vae_state_dict(checkpoint_path: str | Path) -> dict[str, torch.Tensor]:
    state_dict: dict[str, torch.Tensor] = {}
    with safe_open(str(checkpoint_path), framework="pt", device="cpu") as checkpoint:
        for key in checkpoint.keys():
            targets = _diffusion_vae_targets(key)
            if not targets:
                continue
            value = checkpoint.get_tensor(key)
            if len(targets) == 3:
                values = value.chunk(3, dim=0)
                state_dict.update(dict(zip(targets, values, strict=True)))
            else:
                state_dict[targets[0]] = value
    return state_dict


def ltx25_diffusion_vae_checkpoint_key_coverage(
    checkpoint_path: str | Path,
    model_keys: set[str],
) -> tuple[set[str], set[str]]:
    """Return unexplained source keys and missing isolated DiffVAE keys."""
    mapped: set[str] = set()
    with safe_open(str(checkpoint_path), framework="pt", device="cpu") as checkpoint:
        for key in checkpoint.keys():
            mapped.update(_diffusion_vae_targets(key))
