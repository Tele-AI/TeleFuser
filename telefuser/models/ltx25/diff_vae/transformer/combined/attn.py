...
from telefuser.models.ltx25.diff_vae.transformer.attention import NeighborhoodAttention3D
from telefuser.models.ltx25.diff_vae.transformer.rope_math import (
    h_positions,
    rot_abs_axis_impl,
    t_positions,
)

# ``nested_compile_region`` was introduced after some Torch versions TeleFuser
# still supports. Eager execution has the same semantics without the wrapper.
_nested_compile_region = getattr(torch.compiler, "nested_compile_region", lambda function: function)
_rot_abs_axis = _nested_compile_region(rot_abs_axis_impl)
