...
Use :func:`convert_decoder_weights_to_channels_last_3d` before
:func:`enable_memory_efficient_decode` to match the official decoder path.
"""
...
def convert_decoder_weights_to_channels_last_3d(decoder: VideoDecoder) -> VideoDecoder:
    """Convert Conv3d decoder weights to the upstream channels-last layout."""
    for parameter in decoder.parameters():
        if parameter.dim() == 5:
            parameter.data = parameter.data.to(memory_format=torch.channels_last_3d)
    return decoder
