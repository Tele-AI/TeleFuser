"""Isolated LTX-2.5 convolutional video VAE implementation."""
...
from .transformer import PixArtAlphaCombinedTimestepSizeEmbeddings
...
def _ltx_video_vae_kwargs_from_config(config: dict) -> dict:
    ...
    return {
        ...
        "encoder_blocks": tuple(config.get("encoder_blocks", DEFAULT_ENCODER_BLOCKS)),
        "decoder_blocks": tuple(config.get("decoder_blocks", DEFAULT_DECODER_BLOCKS)),
        "patch_size": config.get("patch_size", 4),
        "norm_layer": NormLayerType(config.get("norm_layer", "pixel_norm")),
        "latent_log_var": LogVarianceType(config.get("latent_log_var", "uniform")),
...
class LTXVideoVAE(BaseModel):
    ...
    def __init__(
        ...
        encoder_blocks: tuple[tuple[str, int | dict], ...] = DEFAULT_ENCODER_BLOCKS,
        decoder_blocks: tuple[tuple[str, int | dict], ...] = DEFAULT_DECODER_BLOCKS,
        patch_size: int = 4,
        norm_layer: NormLayerType = NormLayerType.PIXEL_NORM,
        latent_log_var: LogVarianceType = LogVarianceType.UNIFORM,
...
class LTX25ConvVideoVAE(LTXVideoVAE):
    """Isolated LTX-2.5 Conv VAE wrapper with strict split-checkpoint loading."""

    @classmethod
    def from_config(cls, config: dict) -> "LTX25ConvVideoVAE":
        return cls(**_ltx_video_vae_kwargs_from_config(config))
    ...
    def from_checkpoint(
        ...
        *,
        ...
        """Construct and strictly load the official standalone Conv VAE checkpoint."""
        checkpoint = inspect_checkpoint(checkpoint_path)
        with torch.device("meta"):
            model = cls.from_config(checkpoint.config)
        unexpected, missing = ltx25_conv_video_vae_checkpoint_key_coverage(checkpoint.path, set(model.state_dict()))
        if unexpected or missing:
            raise ValueError(
                "LTX-2.5 Conv VAE checkpoint coverage mismatch: "
                f"unexpected={sorted(unexpected)[:5]}, missing={sorted(missing)[:5]}"
            )
        state_dict = _load_ltx25_conv_video_vae_state_dict(checkpoint.path)
        missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=True, assign=True)
        if missing_keys or unexpected_keys:
            raise ValueError(
                f"LTX-2.5 Conv VAE load mismatch: missing={missing_keys[:5]}, unexpected={unexpected_keys[:5]}"
            )
        model = model.to(device=device, dtype=torch_dtype).eval()
        from .memory_efficient_decode import (
            convert_decoder_weights_to_channels_last_3d,
            enable_memory_efficient_decode,
        )

        convert_decoder_weights_to_channels_last_3d(model.decoder)
        enable_memory_efficient_decode(model.decoder)
        return model


def _conv_video_vae_key_to_model_keys(key: str) -> Iterable[str]:
    if key.startswith("encoder.") or key.startswith("decoder."):
        return (key,)
    if key.startswith("per_channel_statistics."):
        suffix = key.removeprefix("per_channel_statistics.")
        return (f"encoder.per_channel_statistics.{suffix}", f"decoder.per_channel_statistics.{suffix}")
    return ()


def _load_ltx25_conv_video_vae_state_dict(checkpoint_path: str | Path) -> dict[str, torch.Tensor]:
    state_dict: dict[str, torch.Tensor] = {}
    with safe_open(str(checkpoint_path), framework="pt", device="cpu") as checkpoint:
        for key in checkpoint.keys():
            targets = tuple(_conv_video_vae_key_to_model_keys(key))
            if not targets:
                continue
...
def ltx25_conv_video_vae_checkpoint_key_coverage(
    checkpoint_path: str | Path,
    model_keys: set[str],
) -> tuple[set[str], set[str]]:
    """Return unexplained checkpoint keys and missing Conv VAE model keys."""
    mapped: set[str] = set()
    with safe_open(str(checkpoint_path), framework="pt", device="cpu") as checkpoint:
        for key in checkpoint.keys():
            mapped.update(_conv_video_vae_key_to_model_keys(key))
