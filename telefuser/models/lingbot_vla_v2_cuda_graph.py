...
class LingBotVlaV2CudaGraphs:
    """Build dynamic prefixes eagerly and replay the fixed denoising graph."""
    ...
    def run(
        ...
        """Build the request-specific prefix, then replay fixed-shape denoising."""
        with self._lock:
            current_stream = torch.cuda.current_stream(state.device)
            if self._completion_event is not None:
                current_stream.wait_event(self._completion_event)
            prefix_pad_masks, prefix_position_ids, past_key_values = self.model.build_prefix_cache(
                images,
                img_masks,
                lang_tokens,
                lang_masks,
