...
启动前设置 `TF_MODEL_ZOO_PATH`；如有需要，再调整 `PPL_CONFIG["parallelism"]`。默认配置使用四张 GPU，并由 TeleFuser 在服务内部创建 Ulysses worker，因此启动命令不需要 `torchrun` 或分布式环境变量。
...
| 参数 | 影响 |
|------|------|
| `PPL_CONFIG["resolution"]` | 选择预设输出面积；可使用内置 `240p` 预设降低显存 |
