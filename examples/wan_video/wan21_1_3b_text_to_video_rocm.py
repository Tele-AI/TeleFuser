...
PPL_CONFIG = dict(
    ...
    tiled=True,
    target_fps=16,
    # 2-tile VAE decode geometry: on a 60x108 (480p 16:9) latent this covers the
    # full frame with only ~1.4x redundant compute instead of the ~2.9x of the
    # default (30, 52)/(15, 26) 12-tile layout, cutting decode time by ~45% on a
    # 16GB gfx1201 while staying within ~7.7GiB of VRAM.
    vae_tile_size=(60, 62),
    vae_tile_stride=(30, 54),
    sample_solver="euler",
    # ROCm has no flash-attn / SageAttention / tf-kernel support; SDPA is natively available.
...
def get_pipeline(parallelism=1, model_root=PPL_CONFIG["model_root"]):
    ...
    """
    # Load models from the official (non-Diffusers) checkpoint layout.
    # low_cpu_mem_usage avoids duplicating the state dict in host RAM while loading.
    ...
    # The ~10.6GB bf16 text encoder stays in host RAM and moves to the GPU only
    # during text encoding, keeping resident VRAM within a 16GB budget.
    pipe_config.text_encoding_config.offload_config.offload_type = WeightOffloadType.MODEL_CPU_OFFLOAD
    # Pageable (non-pinned) CPU copies: the default pinned offload would page-lock
    # ~10.6GB of host RAM, which exceeds a 16GB WSL2 VM together with the DiT/VAE
    # and triggers the kernel OOM killer.
    pipe_config.text_encoding_config.offload_config.pin_cpu_memory = False
    # torch.compile is not validated on ROCm; run eager by default.
    pipe_config.dit_config.compile_config.enabled = False
    if parallelism > 1:
...
def run(
    ...
    """
    Convert text prompts to video sequences using the Wan2.1 1.3B model.
    Args:
        pipeline (Wan21VideoPipeline): Preloaded pipeline object
        prompt (str): Positive guidance text prompt
        negative_prompt (str, optional): Negative guidance prompt merged with the base negative prompt
        seed (int, optional): Random seed. Default is 42
        resolution (str, optional): Resolution such as "480p", "720p"
        aspect_ratio (str, optional): Aspect ratio such as "16:9"
    ...
    video = pipeline(
        ...
        tile_size=PPL_CONFIG["vae_tile_size"],
        tile_stride=PPL_CONFIG["vae_tile_stride"],
        height=height,
        width=width,
...
def main(
    ...
    """Text to video conversion using Wan2.1 1.3B on AMD ROCm GPUs"""
